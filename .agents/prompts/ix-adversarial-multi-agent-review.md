...
1. ix-test-auditor â€” test and typecheck gate
2. ix-git-reviewer â€” commit hygiene, diff scope, upstream pollution
3. ix-security-reviewer â€” dependency, OSS/Pro boundary, OSS boundary
4. ix-documentation-reviewer â€” doc accuracy, CLAUDE.md staleness, contamination
5. ix-pr-reviewer â€” adversarial code review of the PR diff
6. ix-final-reviewer â€” GO/NO-GO synthesis

For fork infrastructure work (this session's scope):
1. ix-git-reviewer - commit hygiene and diff scope
2. ix-final-reviewer â€” GO/NO-GO synthesis (no upstream pollution risk)
...
- The final reviewer overrides no specialist verdict â€” it synthesizes
...
- [ ] `git diff origin/upstream/main...HEAD` inspected â€” no fork-local leakage
...
Verdict contract
----------------
- `GO` â€” all prerequisites met, exceptions documented, commit may proceed
- `NO-GO` â€” at least one blocking item, exact item named, work halted
...
- `scope`, `as_of`
- `specialist_outputs` â€” one block per specialist with verdict
- `conflicts` â€” any disagreements between specialists, with resolution
- `prerequisites_checklist` â€” boolean per item above
- `exceptions` â€” anything unverified, backend: UNVERIFIED, etc.
- `verdict` (GO | NO-GO) with one-sentence mandatory rationale
- `follow_up` â€” exact next step on GO; exact blocking item on NO-GO

Constraints
-----------
- A NO-GO verdict is not negotiable within this prompt â€” it halts the write
