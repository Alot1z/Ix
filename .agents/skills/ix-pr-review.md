...
Steps:
0. Run the `ix-pr-reviewer` mandatory gate first: current-tip fetch, full
   timeline (every commit + comment + reply), temporal finding ownership, and
   duplicate-fix detection (validate an existing contributor fix — do not write
