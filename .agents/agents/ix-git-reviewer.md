...
- No attribution footers in any commit â€” check `git log --format="%B" <sha>` for
...
- `branch_health` â€” exists, tracking, ahead/behind upstream
- `merge_base` (SHA) â€” confirms branch divergence point
- `commit_register` â€” commits on branch vs upstream/main, prefix breakdown
- `diff_against_upstream` â€” changed files, fork-local files present?
- `diff_against_fork_main` â€” what differs from current fork/main tip
- `attribution_scan` â€” any forbidden footer in any commit message
- `convention_audit` â€” prefix rule compliance, message quality
- `conflict_risk` â€” mergeable? any open conflicts? mergeStateStatus
- `verdict` (CLEAN | NEEDS_SQUASH | NEEDS_REBASE | BLOCKED)
- `recommended_actions` â€” ordered exact commands where applicable

Review responsibility
---------------------
A BLOCKED verdict must include the exact reason and the exact remediation steps.
The git reviewer must not be silent â€” always emit verdict.
