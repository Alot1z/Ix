...
Evidence sources
----------------
- `gh api repos/ix-infrastructure/Ix/commits` â€” latest commit SHA
...
- Do not mix Ix-findings review with fork engineering review â€” separate artifacts

Output contract
---------------
A findings-review report containing:
- `review_id`, `reviewer`, `as_of`
- `upstream_freshness` â€” latest commit SHA, PR count, issue count vs. `knowledge.md`
- `findings_status` â€” LOCAL_ABSENT | FRESH | STALE | INCONSISTENT
- `discrepancies` â€” list of mismatches (counts, IDs, titles, dates)
- `consistency_checks` â€” entity references in graph.json that resolve to
  PR/issue numbers no longer existing upstream (gh API cross-check)
- `publication_readiness` â€” APPROVED | NEEDS_REFRESH | NEEDS_RESEARCH
- `push_required` (boolean) â€” whether a findings refresh+push is the correct action
